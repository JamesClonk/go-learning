...
func (rect Rectangle) Draw(img draw.Image, x, y int) error {
	...
	x0 := clamp(1, rect.Rectangle.Min.X, 4096)
	x1 := clamp(1, rect.Rectangle.Max.X, 4096)
	y0 := clamp(1, rect.Rectangle.Min.Y, 4096)
	y1 := clamp(1, rect.Rectangle.Max.Y, 4096)
	x0 += x
	x1 += x
	y0 += y
	y1 += y
	...
	if !rect.Filled {
		drawLine(img, image.Point{x0, y0}, image.Point{x1, y0}, fill) // top
		drawLine(img, image.Point{x0, y1}, image.Point{x1, y1}, fill) // bottom
		drawLine(img, image.Point{x0, y0}, image.Point{x0, y1}, fill) // left
		drawLine(img, image.Point{x1, y0}, image.Point{x1, y1}, fill) // right
	} else {
		draw.Draw(img, image.Rect(x0, y0, x1+1, y1+1), &image.Uniform{fill}, image.Pt(x0, y0), draw.Src)
	}
